...
def coefficient_calculator() -> (
    tuple[CoefficientCalculator, dict[str, pd.DataFrame | pd.Series], pd.DataFrame]
):
    ...
    calculator = CoefficientCalculator(
        data_dict, coef_map, adp_enabled=False, fill_invalid=True
    )
...
def test_compute_coefficients(
    coefficient_calculator: tuple[
        CoefficientCalculator, dict[str, pd.DataFrame | pd.Series], pd.DataFrame
    ],
) -> None:
...
def test_skipped_coefficients(
    ...
    skipped_coefficients = ["coef_invalid_var", "coef_empty"]

    # Check that all expected coefficients were skipped
    for coef_name in skipped_coefficients:
        assert (
            coef_name not in computed_coefficients
        ), f"{coef_name} was computed but should be skipped"


def test_coefficient_simple_addition(
    coefficient_calculator: tuple[
        CoefficientCalculator, dict[str, pd.DataFrame | pd.Series], pd.DataFrame
    ],
) -> None:
    ...
    pd.testing.assert_frame_equal(
        coef_value.astype(np.float64), expected.astype(np.float64), atol=1e-10
    )


def test_coefficient_complex_operations(
    coefficient_calculator: tuple[
        CoefficientCalculator, dict[str, pd.DataFrame | pd.Series], pd.DataFrame
    ],
) -> None:
...
def test_coefficient_nan_handling(
    coefficient_calculator: tuple[
        CoefficientCalculator, dict[str, pd.DataFrame | pd.Series], pd.DataFrame
    ],
) -> None:
...
def test_coefficient_division_by_diagonal(
    coefficient_calculator: tuple[
        CoefficientCalculator, dict[str, pd.DataFrame | pd.Series], pd.DataFrame
    ],
) -> None:
...
def test_coefficient_division_by_sparse(
    coefficient_calculator: tuple[
        CoefficientCalculator, dict[str, pd.DataFrame | pd.Series], pd.DataFrame
    ],
) -> None:
